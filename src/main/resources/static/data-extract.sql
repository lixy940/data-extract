...
CREATE TABLE `commit_table_record` (
  ...
  `data_type` tinyint(2) DEFAULT '1' COMMENT '数据类型 1 string, 2 日期 , 3 日期字符串',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`commit_id`)
) ENGINE=InnoDB AUTO_INCREMENT=104 DEFAULT CHARSET=utf8mb4 COMMENT='已提交的表记录';
...
CREATE TABLE `handler_task` (
  ...
  `task_period` varchar(128) DEFAULT NULL COMMENT '任务周期',
  `task_type` tinyint(2) DEFAULT NULL COMMENT '任务类型 1 永久一次,2 周期执行',
  `task_status` tinyint(2) DEFAULT '0' COMMENT '务状态，0 未开始，1 进行中，2 已完成',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`task_id`)
) ENGINE=InnoDB AUTO_INCREMENT=207 DEFAULT CHARSET=utf8mb4 COMMENT='任务列表';
...
CREATE TABLE `handler_task_status` (
  ...
  `status` tinyint(2) DEFAULT '1' COMMENT '状态  1 进行中 , 2 已完成',
  `execute_result` tinyint(2) DEFAULT NULL COMMENT '执行结果 0 失败 1 成功',
  `begin_index` varchar(64) DEFAULT NULL COMMENT '开始序列',
  `end_index` varchar(64) DEFAULT NULL COMMENT '结束序列',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`handler_id`)
) ENGINE=InnoDB AUTO_INCREMENT=2778 DEFAULT CHARSET=utf8mb4 COMMENT='任务处理详情';
...
CREATE TABLE `sys_dbinfo` (
  ...
  UNIQUE KEY `db_unique_index` (`db_type`,`db_ip`,`db_port`,`db_server_name`,`db_table_schema`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=56 DEFAULT CHARSET=utf8 COMMENT='数据库';
